...
export function initVisionClient() {
  try {
    ...
    try {
      ...
      // Third try: Try to base64 decode (some systems encode JSON as base64)
        ...
        try {
          ...
          if (decodedJson.trim().startsWith('{')) {
            ...
            console.log('Google Vision client initialized with base64-decoded credentials');
          } else {
            // If not valid JSON, try without explicit credentials
            visionClient = new ImageAnnotatorClient();
            console.log('Base64 decode did not result in valid JSON, using default credentials');
          }
        } catch (base64Error) {
      ...
      visionClient.auth.getAccessToken().then(() => {
        console.log('Google Vision client initialized and authenticated successfully');
      }).catch((authError) => {
        console.warn('Google Vision client initialized but authentication failed:', authError.message);
      });
      
      return visionClient;
    } catch (parseError) {
      console.error('Failed to parse or use Google Cloud credentials:', parseError);
      
      // Last attempt: try to initialize without explicit credentials
      // (relies on GOOGLE_APPLICATION_CREDENTIALS environment variable)
      try {
        visionClient = new ImageAnnotatorClient();
        console.log('Google Vision client initialized without explicit credentials');
        
        // Test authentication
        visionClient.auth.getAccessToken().then(() => {
          console.log('Google Vision client authentication successful with default credentials');
        }).catch((authError) => {
          console.warn('Google Vision authentication failed with default credentials:', authError.message);
        });
...
export async function extractTextFromImage(imagePath) {
  ...
  if (!client) {
    console.warn('Google Vision client not initialized, cannot extract text from image');
  ...
  try {
    ...
    if (imageStats.size > maxFileSizeBytes) {
      console.warn(`Image file too large (${Math.round(imageStats.size / (1024 * 1024))} MB), may fail OCR`);
    }
    ...
    const ocrPromise = client.textDetection(imageContent)
      .then(([result]) => {
        ...
        console.log(`Successfully extracted ${fullText.length} characters from image`);
...
export async function extractTextFromPdf(pdfPath) {
  ...
  if (!client) {
    console.warn('Google Vision client not initialized, cannot extract text from PDF');
    return 'Google Vision OCR service not available. Please check your Google Cloud credentials.';
  }
  ...
  try {
    ...
    if (fileStats.size > maxFileSizeBytes) {
      console.warn(`PDF file too large (${Math.round(fileStats.size / (1024 * 1024))} MB), may fail OCR`);
    ...
    const ocrPromise = client.documentTextDetection(pdfContent)
      .then(([result]) => {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error('Error extracting text from PDF with Google Vision:', errorMessage);
